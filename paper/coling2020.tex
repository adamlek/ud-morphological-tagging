...
\end{abstract}
    ...
    %explore how
    % ``universal''  different computational methods are.
...
%
	...
	\end{equation}
     ...
     %            The above operation produces one BPE-contextualized
     %vector per word, which is passed to an LSTM, to take into account
     %the word context. That is, the contextual information that the
     %Transformer provides is in terms of BPE tokens, while the LSTM
     %takes context words (BPE tokens composed into words) into
     %account.
    %
...
%     representations, to regularize this operation we apply dropout
...
\begin{figure}%{r}{8cm}
    ...
    %
     ...
     % which is not strictly necessary since we can pool this information.
     % JP: I think that this is ala

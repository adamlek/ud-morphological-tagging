...
\begin{document}
	...
	\label{intro}
            ...
            After its introduction \adam{[CITE:TODO]}, the transformer model
     ...
     \citep{devlin2018bert}\jp{???}. For our purposes, the goal in sequence
    ...
    %\end{table}
     ...
     the number of parameters, and then feeding them with correspondingly
...
\begin{tikzpicture} 
    ...
    \caption{\label{fig:bpe_lens} Per-language accuracy on tokens
     ...
     tokens who are composed of seven or more BPE tokens.\jp{???} We indicate
     ...
     calculated on. We show the the Agresti-Coull approximation of a 95\%-confidence interval for the RNN
     method \citep{agresti1998approximate}.}
    ...
    %
         Analogously to character embeddings\jp{what is this referring to?} using a method that takes
     the order of elements into account seems to be able to collect
    ...
    We conjecture that this discrepancy is can
     ...
     morphology (and by extension, semantics).\jp{actually this is not an example of non-commutativity; for that you'd need ``le+ab'' to be occuring in English, but it really isn't (as far as I know.)}

    
       In general, we see that the RNN performs better
     than summation or averaging, but one question that arises when we
    ...
    \subsubsection{Fst method}
     ...
     method proposed by \citet{devlin2018bert} is less efficient than any other method we tested, for
     ...
     accuracy of tokens composed of more than two BPE token in
     ...
     much lager gap in performance.
             ...
             While the fst\jp{typeset} method may be effective, for classifying
    ...
    \subsubsection{Sum and Mean}
     ...
     advantage of $.9$ percentage points for summation.
            ...
            This shows \jp{??} by averaging, we are removing some predictive
...
\begin{figure}%{r}{8cm}
    ...
    \section{Conclusions and Future Work}
     ...
     the remaining UD treebanks to our experiments\jp{how many are there?}.  Additionally, an
     interesting direction for future work is to explore BPE
     compositions for other syntactic and semantic tasks.

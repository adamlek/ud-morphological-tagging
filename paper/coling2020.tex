...
\begin{document}
	...
	\label{intro}
     ...
     aims to find relatively large sub-word units occurring
     significantly often, while maximizing the coverage of the corpus
     (the presence of the ``out of vocabulary'' token is
     minimized). Typical approaches to composing tokens from sub-token
     ...
     \textit{morphemes}
     \citep{el2012orthographic,chaudhary2018adapting,xu2017implicitly},\jp{missing
       citations?}  and then combining them. In this paper, we
     ...
     rather it aims at splitting a corpus $\mathcal{C}$ into $N$
     tokens, where $N$ is user defined. [EXPAND]
     ...
     different orthographically and grammatically, one can expect the
     methods to derive information not to be uniformly
     efficient. However, at the same time, researchers always strive
     ...
     languages and solve tasks. This is precisely what we do here: we
     search for the best methods for BPE combination, and analyse them
     in various contexts.
	...
	\label{results}
     ...
     Even though our aim is to compare the relative performance
     of various BPE combination methods rather than to improve on the
     ...
     \citet{mccarthy2019sigmorphon,kondratyukstraka}, use tree-bank
     concatenation, which means that results are not reported on a
     per-treebank basis --- contrary to what we do here.
...
\begin{tikzpicture} 
    ...
    \caption{\label{fig:bpe_lens} Per-language accuracy on tokens with
     ...
     the $x$-axis indicate how many samples the accuracy was calculated on. }
    ...
    % the wild'' this cannot be incorporated into the training and
    % must be analysed with feature extraction. 

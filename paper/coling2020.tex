...
\begin{document}
    ...
    %
     ...
     word representation of ``scientifically'' which we can assign a label to.
...
%
	...
	\end{wraptable}
     ...
     representations in the layer to $0$. After the word-LSTM has
    ...
    \end{table}
     ...
     for Basque and $8.8$ points for Turkish.\jp{In this paragraph we don't know the reference for ``increase'' or ``decrease''}
    %
...
\end{table}
    ...
    \cref{tab:results_large_tokens}.
     ...
     where the RNN outperforms the Fst method\jp{all other methods?}, and both the summation and averaging methods. We
...
\begin{tikzpicture} 
	...
	\end{tikzpicture}
     ...
     blue and RNN as red. The accuracy is given on the $y$-axis. We
    ...
    %
     ...
     that compose a word, as is thus able to retain the information
     ...
     the large difference between finetuning and feature extraction
...
\begin{figure}%{r}{8cm}
    ...
    % tokenization that perform some type of word classification. 
     ...
     tranformer with BPE tokenization, combining BPE tokens into word
     representations using an RNN performs better than two commutative
     methods, summation, averaging, and taking the first BPE token as
     the word representation.
    
    As future work we want to continue experimenting with the different

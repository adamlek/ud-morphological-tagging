...
\begin{document}
    ...
    %\end{table}
     ...
     different (morpho-)syntactically, one can expect the methods to
     derive information not to be uniformly efficient.
...
%	\end{table}
    ...
    \end{table}
     ...
     trend we see that the RNN method tends to perform better than the
     first bpe token, summation or averaging methods. This is
     ...
     of the RNN is small, it occurs consistenty.\jp{Can we rule out
     the possibility that this is simply due to additional
     non-recurrent layers? One could say that the two additional dense
     layers already do this job and so the non-recurrent aspect is
     already taken care of by these last two layers(?)}
    %
...
\begin{tikzpicture} 
    ...
    \caption{\label{fig:bpe_lens} Per-language accuracy on tokens
     ...
     the method by encoding fst as pink, summation as green, averaging as blue and
    ...
    \subsection{BPE composition methods}
        For our experiments we see that the RNN method is most
     effective than the other proposed methods, summing, averaging or
     taking the first BPE token, for morphological feature
     prediction. We see this holds regardless of training regime
     (finetuning versus feature extraction) and across languages with
     different BPE per token ratios.
    %
         Analogously to character embeddings, using a method that take
     the order of elements into account seem to be able to collect
     more predictive information from sub-word embeddings.
    We conjecture that this discrepancy is can
     ...
     morphology (and by extension, semantics).
       ...
       In general, we see that the RNN perform better
     than summation or averaging, but one question that arise when we
     look at \cref{fig:bpe_lens}, specifically looking at the
     performance on tokens composed of only one BPE token, is whether
     this is because of the additional parameters in the model, or the
     ...
     RNN, effectively doing a non-linear transfromation with tanh
     ...
     subset of the composite tag to the BPE token.
                ...
                We can suspect at this points that the average
     BPE-per-token ratio in a language affect the composition method
     used. In \cref{fig:scatter_len} we plot the average number of BPE
     tokens per text token, and compare this against the gain in
     accuracy yielded by using the RNN method over summation.  For
     fine-tuning we see that in general the average number of BPE
     tokens do not matter than much. The two cases where it does
     matter is for Turkish and Basque, where we see a substantial
     improvement of about $3$ percentage points. For the other
     languages the improvements lie in the range $.6$ to $1.2$. This
     indicate that when fine-tuning, the model can provide information
     that allows commutative methods to properly compose BPE tokens.
      However, looking at feature extraction we see that there is a
     much larger divide between the low-bpe-ratio languages and the
     ...
     agglutinative languages, but the typology does not appear to have
    ...
    \subsubsection{Fst method}
                        ...
                        Our experiments reveal that the standard
     method proposed in \citep{devlin2018bert} is not efficient for
     morphological sequence classification across languages. We see in
     \cref{tab:results_tokens} that the method is, on average, $.4$
     and $1$ percentage points lower than the second lowest scoring
     method for both finetuning and feature extraction respectively.
      We see that this effect is further enhanced when we consider the
     accuracy on tokens composed of more than two BPE token in
     \cref{tab:results_large_tokens}, where the difference is $1.6$
     and $2.7$ percentage, compared against the second lowest scoring
     ...
     an implicit objective to the model that is not strictly
     necessary, since we can pool information from the other tokens
    ...
    \subsubsection{Sum and Mean}
     ...
     any of them when doing the fine-tuning regime. However, when only
     extracting features we see some hints that summation is more
     effective than averaging. For feature extraction, summation is
     $.5$ percentage points better than averaging, and when we
     consider tokens composed of two or more BPE tokens this shows an
     advantage of $.9$ percentage points for summation.

            This shows by averaging, we are removing some predictive
     information from the pretrained BPE tokens, that is, by reducing the values
     in the BPE embeddings uniformly across a collection of BPE
     embeddings we lose useful information. This suggests, perhaps not
     surprisingly, that some BPE embeddings contain more predictive
...
\begin{figure}%{r}{8cm}
    ...
    \section{Conclusions and Future Work}
     ...
     Recurrent Neural Network compared to taking the firs,
     ...
     constituency parsing. Another avenue we wish to explore is adding
     the remaining UD treebanks to our experiments.  Additionally, an
     interesting direction for future work is to also to explore BPE

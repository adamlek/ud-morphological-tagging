...
\begin{document}
	...
	\label{intro}
     ...
     \citet{vaswani2017attention} has emerged as the dominant
     ...
     \citet{bojanowski2017enriching}, while other approaches have
     ...
     \citet{sennrich2015neural}, which take another approach. It does
     ...
     \citet{devlin2018bert}. For our purposes, the goal in sequence
    ...
    \section{Data}
     ...
     dataset \citet{nivre2018} annotated with the UniMorph schema
     \citet{mccarthy2018marrying}. We are interested both in the
	...
	\subsubsection{BPE feature extraction}
     ...
     \citet{kondratyukstraka,raganato2018analysis} [Find 1 or 2 more
     ...
     representations \citet{kondratyukstraka}.  That is, we initialize
...
\begin{tikzpicture} 
    ...
    \subsubsection{Sum and Mean}
     ...
     summation or averaging and, on a global scale, our experiments
     support these findings.

        However, looking closer at some of the results we obtained we
     find that summation and averaging matter, more for some languages
     than others. In general, these differences appear when we only
     extract features from the transformer model, in this case we see
     a difference of $1.2$ percentage points on average between the
     methods, where summation often performs better. A particular
     striking example is Arabic where the gain is $2.1$ percentage
     points.
     ...
     model are probed for linguistic knowledge). Relatedly, a
     published resource using the transformer will most likely be
     trained using finetuning, however, as it receive new data ``in
     the wild'' this cannot be incorporated into the base-model and
     must be analysed with feature extraction. And when this is the
     case we have shown that the composition method do matter, whether
     it's summation, averaging or using an RNN.
	...
	\section*{Acknowledgments}
    	Something goes here roughly of this length. Something goes
     here roughly of this length. Something goes here roughly of this
     length. Something goes here roughly of this length. Something
     goes here roughly of this length. Something goes here roughly of
     this length.

...
\begin{tikzpicture} 
    ...
    %    To some extent, when using BPE tokenization languages
    % morphological typology is converted to fusional.  This is because
    % one BPE token may consist of several morphemes is a agglutinative
    % language and are interpreted as one meaningful language unit (as
    % a representation is assigned to the unit). Thus, when we predict
    % a label we have to map this one unit (that in reality is two or
    % more units) to $1$ tag, while in-fact it should be mapped to $k$
    % tags.
    ...
    (1.77,0.861928171)
        ...
        %xlabel=BPE-token ratio,
        %ylabel=RNN accuracy - mean accuracy
		]
    \addplot[
          color=blue,
	      mark=square*,
	      ...
	      coordinates {
    (1.98,0.047574944999999924)
    (1.79,0.057908772)
    (1.73,0.03495608699999997)
    (1.86,0.046201833333333275)
    };
\addplot[
              color=red,
	      mark=square*,
	      ...
	      coordinates {
(1.39,0.015275315666666622)
(1.25,0.009607613000000015)
(1.75,0.040448118999999894)
(1.77,0.04729016033333333)
};
            \legend{Agglutinative, Fusional}
    		\end{axis}
        \end{tikzpicture}
           \caption{\label{fig:bpe_len_acc} Figure (a) show the
     performance of the different method compared against the average
     BPE-to-token ratio using the finetuning training regime. Figure (b) show the gain of using an RNN as
     compared to the other methods, we calculate this by subtracting
     the accuracy of the RNN method by the mean of the other three
     methods accuracy.}
    \end{figure}

...
\end{abstract}
    ...
    \section{Task}
            ...
            To investigate composition methods for BPE token vectors
...
%
	...
	\end{equation}
     ...
     \subsubsection{Word-level feature sets and classification}
         The above operation produces one contextual vector per word,
     which is passed to an LSTM (regardless of the method), to take
     into account the word context. That is, the contextual
     information the Transformer provides in in terms of BPE tokens,
...
\begin{tikzpicture} 
	...
	\end{tikzpicture}
     ...
     tokens with different numbers of BPE components, for the
     ...
     $y$-axis. We show the the Agresti-Coull approximation of a
...
\begin{figure}%{r}{8cm}
    ...
    \subsection{Parameterization of First, Sum and Mean}
     ...
     %words composed of only one BPE token, is whether  is due to
     ...
     only one BPE token, is whether the RNNs performance is better due
     the contextual information it adds or that it contains more
     parameters and thus are able to model the data better.
    %
    ...
    %
     ...
     representations of the contextual words.
         ...
         We test this by parameterizing the First, Sum, and Mean
     method.  This is done by passing all BPE vectors throught a
     non-linear transformation with $\mathsf{ReLU}$ activation before
     we compute the Sum, Mean, or select the first BPE-token.
    
    %This is done by adding a non-linear transformation
    % with $\mathsf{ReLU}$ activation before we compute the Sum, Mean,
    % or select the first BPE-token.
    ...
    % tokenization that perform some type of word classification. 
     ...
     information into a single BPE token vector.
    %
    ...
    % Lexical probes om pretrained word-embeddings using the First model will be bad.
